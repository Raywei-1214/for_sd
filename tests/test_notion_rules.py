...
class NotionRulesTests(unittest.TestCase):
    ...
    def test_probe_context_shallow_shell_detects_bare_explore_shell(self) -> None:
        service = RegistrationService.__new__(RegistrationService)

        self.assertTrue(
            RegistrationService._is_probe_context_shallow_shell(
                service,
        ...
        self.assertFalse(
            RegistrationService._is_probe_context_shallow_shell(
                service,
                "url=https://dreamina.capcut.com/ai-tool/home?type=video&workspace=0 | body=Explore Create Assets Canvas 0 Upgrade Start Creating With AI Agent AI Image AI Video Reference Upload up to 12 references",
            )
        )
