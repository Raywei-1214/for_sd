...
- 填完验证码后，当前会显式触发一次确认页收敛：
  - 先做一次 `Tab` 失焦
  - 再补一次 `Enter` 提交
  - 只在确认页停留时执行，不额外追加外网请求
...
- `fill_profile` 当前已改为专用等待链路：
  ...
  - 若验证码页仍未真正提交，会在同页内补一次确认页收敛，再继续等待资料页出现
  - 失败时会额外记录：
    - `signup_form=visible`
...
- `probe` 当前会额外识别 `Explore Create Assets` 这类浅壳视频页：
  ...
  - 会先做一次轻量清场，再在同页短等后复采
  - 不会追加新的 `goto`
- `probe` 当前也会识别 `AI Agent Auto Trends / Creative Partner Program / prompt guide` 这类软阻塞壳子：
  - 若仍是视频页，但正文被这类软阻塞文案覆盖
  - 会先做一次轻量清场，再在同页短等后复采
  - 同样不追加新的 `goto`
...
- `probe` 当前只接受“非首页壳子 + 至少采到一个数值型 credits/cost”的结果：
  ...
  - 若 ready 判定未通过，则不会继续把 `seedance2_cost=0`、模型下拉或按钮文本当成可信信号，避免 `missing_credits` 被首页壳伪采样放大
