...
- `open_home` 当前最多只做 `1` 次重试（共 `2` 次尝试），避免因为网络抖动反复重打同一重页面。
...
- `internxt` 当前已补专用提取逻辑：
  ...
  - 验证码阶段会优先尝试站内轻刷新，再点开 `Dreamina / CapCut / verification` 相关邮件正文
...
- 验证码等待当前优先走“站内轻刷新”，整页 `reload` 已降频：
  - 轻刷新周期：每 `4` 轮
  - 整页重载周期：每 `10` 轮
  - 目的：减少慢节点下的整页重复加载流量
...
- 当前故意不拦截：
  ...
  - `xhr/fetch`
  - `stylesheet`
- 目的：优先减少流量，不直接破坏注册主流程依赖的前端脚本和接口请求
