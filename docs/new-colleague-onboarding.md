...
这轮采样都会被判成无效，流程会先优先尝试 `AI Video`，再尝试 `Start Creating` 轻量进入视频工作区；如果 URL 已经偏离视频页，会直接放弃该次采样，不再额外追加一次 `goto`。如果页面只剩 `Explore Create Assets` 这类半 ready 壳子，会补一次轻量工作区引导再采。
